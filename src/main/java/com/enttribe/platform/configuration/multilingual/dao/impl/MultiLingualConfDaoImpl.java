package com.enttribe.platform.configuration.multilingual.dao.impl;
...
public class MultiLingualConfDaoImpl extends GenericDaoImpl<Integer, MultiLingualConfiguration> implements IMultiLingualConfigurationDao {
    ...
    public List<MultiLingualConfiguration> getMultiLingualProps(List<CriteriaFilterParameter> filters,
        ...
        // Implementation for getting multilingual properties
        return null;
    }

    @Override
    public Integer getMultiLingualPropsCount(List<CriteriaFilterParameter> filters) {
        StringBuilder queryString = new StringBuilder("SELECT COUNT(*) FROM MultiLingualConfiguration WHERE isDeleted = false");
        if (filters != null && !filters.isEmpty()) {
            for (CriteriaFilterParameter filter : filters) {
                queryString.append(" AND ").append(filter.getLabelType()).append(" = :").append(filter.getLabelType());
            }
        }
        var query = entityManager.createQuery(queryString.toString(), Long.class);
        if (filters != null && !filters.isEmpty()) {
            for (CriteriaFilterParameter filter : filters) {
                query.setParameter(filter.getLabelType(), filter.getValue());
            }
        }
